...
class Checkpointer:
    ...
    Adapted from https://github.com/facebookresearch/fvcore/blob/main/fvcore/common/checkpoint.py
    """
    ...
    def save(self, name: str, **kwargs: Any) -> None:
        ...
        with path_mgr.open(save_file, "wb") as f:
    ...
    def load(self, path: str, checkpointables: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        if not os.path.isfile(path):
            path = path_mgr.get_local_path(path)
    ...
    def has_checkpoint(self) -> bool:
        ...
        return path_mgr.exists(save_file)
    ...
    def get_checkpoint_file(self) -> str:
        ...
        save_file = os.path.join(self.save_dir, "last_checkpoint")
        try:
            with path_mgr.open(save_file, "r") as f:
    ...
    def get_all_checkpoint_files(self) -> List[str]:
        ...
        all_model_checkpoints = [
            ...
            for file in path_mgr.ls(self.save_dir)
            if path_mgr.isfile(os.path.join(self.save_dir, file)) and file.endswith(".pth")
        ]
    ...
    def tag_last_checkpoint(self, last_filename_basename: str) -> None:
        ...
        """
        save_file = os.path.join(self.save_dir, "last_checkpoint")
        with path_mgr.open(save_file, "w") as f:
    ...
    def _load_model(self, checkpoint: Any) -> _IncompatibleKeys:
        ...
        for k in list(checkpoint_state_dict.keys()):
            if k in model_state_dict:
                ...
                if isinstance(model_param, nn.parameter.UninitializedParameter):
                    continue
        ...
        if (iteration + 1) % self.period == 0:
            ...
            if self.max_to_keep is not None:
                ...
                if len(self.recent_checkpoints) > self.max_to_keep:
                    ...
                    if path_mgr.exists(file_to_delete) and not file_to_delete.endswith(
                        f"{self.file_prefix}_final.pth"
                    ):
                        path_mgr.rm(file_to_delete)
